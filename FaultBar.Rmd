...
```{r}
type.mdl <- glmmTMB(data = fbs.df, LightPlusMedium ~ Type, ziformula= ~1, family = nbinom1)
summary(type.mdl)
```
...
prim.light.poisson.mdl <- glmmTMB(data = fbs.prim.df, LightPlusMedium ~ Habitat, ziformula = ~1, family = nbinom1)
